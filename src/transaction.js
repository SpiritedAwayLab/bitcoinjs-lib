...
Transaction.OVERWINTER_HASH_PERSON = Buffer.concat([
	Buffer.from("ZcashSigHash"),
	Buffer.from("76b809bb", "hex")
]);
Transaction.SAPLING_HASH_PERSON = Buffer.concat([
	Buffer.from("ZcashSigHash"),
	Buffer.from("600eb42b", "hex")
]);
...
var ZC_NOTEPLAINTEXT_SIZE =
	...
	ZC_R_SIZE +
	ZC_MEMO_SIZE;
var ZC_SAPLING_ENCPLAINTEXT_SIZE =
	ZC_NOTEPLAINTEXT_LEADING +
	ZC_DIVERSIFIER_SIZE +
	ZC_V_SIZE +
	ZC_R_SIZE +
...
var ZC_SAPLING_ENCCIPHERTEXT_SIZE =
	ZC_SAPLING_ENCPLAINTEXT_SIZE + NOTEENCRYPTION_AUTH_BYTES;
var ZC_SAPLING_OUTCIPHERTEXT_SIZE =
...
Transaction.fromBuffer = function(buffer, zcash, __noStrict) {
	...
	if (tx.version >= 2 && zcash) {
		...
		for (i = 0; i < jsLen; ++i) {
			...
			for (j = 0; j < Transaction.ZCASH_NUM_JS_OUTPUTS; j++) {
				ciphertexts.push(
					readSlice(Transaction.ZCASH_NOTECIPHERTEXT_SIZE)
				);
			}
		...
		if (
			tx.version >= 4 &&
			tx.spendDescs.length + tx.outputDescs.length > 0
		) {
...
Transaction.prototype.hashForZIP143 = function(
	inIndex,
	prevOutScript,
	value,
	hashType
...
Transaction.prototype.hashForZIP243 = function(
	inIndex,
	prevOutScript,
	value,
	hashType
